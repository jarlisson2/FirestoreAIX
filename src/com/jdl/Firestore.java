...
@DesignerComponent(version = 2, description = "Firebase Firestore <br> Developed by Jarlisson", category = ComponentCategory.EXTENSION, nonVisible = true, iconName = "aiwebres/firestore.png", versionName = "0.1.1")
@UsesLibraries(libraries = "firestore.jar, gson-2.1.jar, support-annotations-28.0.0.jar")
...
public class Firestore extends AndroidNonvisibleComponent {
    ...
    public void init(boolean hasInstance, String projectId, String apiKey, String applicationId, boolean persistent) {
        ...
        if (hasInstance) {
            ...
            FirebaseFirestoreSettings settings = new FirebaseFirestoreSettings.Builder().setPersistenceEnabled(persistent).build();
            db.setFirestoreSettings(settings);
        } else {
            FirebaseOptions options = new FirebaseOptions.Builder().setApiKey(apiKey).setApplicationId(applicationId).setProjectId(projectId).build();

            FirebaseApp app = FirebaseApp.initializeApp(context, options, name);
            db = FirebaseFirestore.getInstance(app);
    ...
    public void InitializeFirebase(String projectId, String apiKey, String applicationId, boolean persistent) {
        ...
        if (!FirebaseApp.getApps(context).isEmpty()) {
            for (FirebaseApp app : apps) {
                ...
                String instance = app.getName();
                instances.add(instance);

                if (index == size) {
                    init(instances.contains(name), projectId, apiKey, applicationId, persistent);
                }
            }
        } else {
            init(false, projectId, apiKey, applicationId, persistent);
        }
    }
